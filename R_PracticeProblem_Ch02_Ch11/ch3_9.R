...
emp_pay<-function(x){
  ...
  i<-1
  for (j in x){
    names[i]<-str_extract(j,'[가-힣]{3}')
    tmp<-str_replace(j,'[0-9]{4}[가-힣]{3}',"")
    pay[i]<-as.numeric(str_extract(tmp,'[0-9]{3}'))
    i=i+1
  }
  avg<-mean(pay)
  cat("전체 급여 평균: ", avg,"\n")
...
plot(iris[,1],iris[,3],xlab="Sepal.Length",ylab="Petal.Length")
  #조건2
plot(iris[,1],iris[,3],col=iris[,5],xlab="Sepal.Length",ylab="Petal.Length")
  #조건3
title(main="iris 데이터 테이블 산포도 차트")
  #조건4
getwd()
setwd("C:/Users/jung5/Desktop/practice")
