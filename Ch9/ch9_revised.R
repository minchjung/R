...
[64-bit] C:\Program Files\R\R-3.6.0
# 정렬 조회 - 나이 칼럼을 기준으로 내림차순 정렬
...
query1 = "insert into test_table values('La', '1233', '라감찬', 99)"
query2 = "insert into test_table values('jung', '1235', '정감찬', 11)"
query3 = "insert into test_table values('seong', '1662', '성감찬', 23)"
query4 = "insert into test_table values('park', '1231', '박감찬', 76)"
query5 = "insert into test_table values('dim', '1256', '딤', 34)"
query6 = "insert into test_table values('scott', '1144', '스캇', 24)"
query7 = "insert into test_table values('lania', '5476', '라니아', 98)"

dbSendUpdate(conn, query1)
dbSendUpdate(conn, query2)
dbSendUpdate(conn, query3)
dbSendUpdate(conn, query4)
dbSendUpdate(conn, query5)
dbSendUpdate(conn, query6)
dbSendUpdate(conn, query7)
...
################################################################################
## MariaDB 10.5 cmd prompt 연결 !! 
###################################
# Setting environment for MariaDB 10.5 (x64)

# C:\Windows\system32>mysql -u scott -p
# Enter password: *****    <- 비번 tiger
...
#income=small, income=large 는 연관성에서 빼라는것 
apr1
...
setwd()
news_data <- read.csv(file ="nes_data.csv", header = T, stringsAsFactors = F)
str(news_data)
