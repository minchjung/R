...
histogram(~gcsescore, data = Chem97) 
             #x축 칼럼정보, #대상 데이터 =>백분율로 분포도가 나타남
                                        

# 실습: score 변수를 조건변수로 지정하여 데이터 시각화하기 
histogram(~gcsescore | score, data = Chem97)
histogram(~gcsescore | factor(score), data = Chem97)
         #(gcsscore x축 칼럼정보 | score변수로 지정 , data지정)=>score범주 6개라 6개 plot이 그려짐 
        # => score 범주에 따라 각 구간별로 백분율 분포도가 나타남 
        # factor로 지정해서 보는 결과가 더 깔끔함 
...
# 단계 4: 막대 그래프 그리기    #그래프는 열행 기반 순서임 
barchart(Var1 ~ Freq | Var2, data = dft, layout = c(4, 1))
# x축:Var1컬럼~Freq까지 | Var2 조건부 , 데이터, 레이아웃 4개의패널을 1행으로!
# Var2 조건의 범주가 4개임 => 4개의 막대 그래프 
...
#도트플랏으로 그룹화시키는것이 가장 시각적으로 분석하기 좋음. 
...
convert <- transform(airquality, Month = factor(Month)) #아예 Month를 파생변수로 
#원 데이터에 추가시켜서 새로운 형식의 convert 데이터 생성 
str(convert)
...
# 산점도 데이터셋으로!  산점도 플랏을 변수로해 업데이트 시키기 
...
#새로운 파생데이터 추가 
#depth2 지각깊이를 넘버링해서 칼럼 추가한것 
# [조건]<-값 : 조건에 맞는값 depth2칼럼에 추가 

# 단계 3: 리코딩된 변수(depth2)를 조건으로 산점도 그리기 
convert <- transform(quakes, depth2 = factor(depth2))
xyplot(lat ~ long | depth2, data = convert,cex=1,col='red')
...
xyplot(Ozone + Solar.R ~ Wind | factor(Month), 
       ...
       col = c("blue", "red"),
       layout = c(5, 1),cex=2)
      #(오존,솔라 두개값을 y축값으로 보겠다~ Wind를 x축| 팩터조건 월추가, 데이터 
#       색은 순서대로 오존 솔라 
#        5열 1행 =>5개 가로로 plot)
...
depthgroup   #depth 칼럼을 5개로 범주화 시킴
...
magnitudegroup                        # mag 칼럼을 2개로 범주화 
...
xyplot(lat ~ long | depthgroup * magnitudegroup, data = quakes, 
       ...
       ylab = "latitude", xlab = "longitude", 
       pch = "@", col = c("red", "blue"))
# depthgroup 5개 범주화됨 x  magnitudegroup 2개로 범주화됨 = 10개로 범주화됨 
...
cloud(depth ~ lat * long, data = quakes, 
      zlim = rev(range(quakes$depth)), #<-회전rev
...
qplot(hwy, data = mpg) # q연비의 빈도가 y값

# 단계 2: fill 속성 적용  => drv조건은  색으로 채워져 표시됨 
qplot(hwy, data = mpg, fill = drv) 

# 단계 3: binwidth 속성 적용
qplot(hwy, data = mpg, fill = drv, binwidth = 2) #=>밴드 두께 2


# 실습: facets 속성을 사용하여 drv 변수값으로 행/열 단위로 패널 생성하기 
# 단계 1: 열 단위 패널 생성
qplot(hwy, data = mpg, fill = drv, facets = . ~ drv, binwidth = 2) # 가로로 나옴 
# drv 컬럼 조건으로 나눠서 plot =>3개나옴
                                                                     
# 단계 2: 행 단위 패널 생성
qplot(hwy, data = mpg, fill = drv, facets = drv ~ ., binwidth = 2) # 세로로 나옴 


# 실습: qplot() 함수에서 color 속성을 사용하여 두 변수 구분하기 
# 단계 1:  두 변수로 displ과 hwy 변수 사용
qplot(displ, hwy, data = mpg) #엔진크기, 연비를 변수로 사용 
...
qplot(wt, mpg, data = mtcars, 
      size = qsec, color = factor(carb), shape = factor(cyl))#4개변수임 
...
getwd()
ggsave(file = "diamond_price.pdf")
ggsave(file = "diamond_price.jpg", dp = 72)
# windo10부터 C:접근자 제한있음 그냥 파일명 저장후 현재 폴더에저장 
...
seoul <- c(left = 128.73, bottom = 34.8799083, 
           right =129.05562775, top = 35.3959361)

# 단계 2: zoom, maptype으로 정적 지도 이미지 가져오기 
map <- get_stamenmap(seoul, zoom = 15, maptype = 'terrain')
ggmap(map)
